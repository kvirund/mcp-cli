/**
 * NASA APOD MCP Server - API Client Service
 */

import { ApodEntry, ApodOutput } from "../types.js";
import { API_BASE_URL, DEFAULT_API_KEY, REQUEST_TIMEOUT } from "../constants.js";
...
async function makeRequest<T>(url: string): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new NasaApiError(
        `NASA API error: ${response.status} - ${errorText}`,
        response.status
      );
    }
...
export function formatApodMarkdown(entry: ApodOutput): string {
  const lines: string[] = [
    `## ${entry.title}`,
    `**Date:** ${entry.date}`,
    `**Type:** ${entry.media_type === "video" ? "🎬 Video" : "🖼️ Image"}`,
    ""
  ];
  ...
  if (entry.hd_url) {
    lines.push(`**HD Version:** ${entry.hd_url}`);
  }
  
  if (entry.thumbnail_url) {
    lines.push(`**Thumbnail:** ${entry.thumbnail_url}`);
  }
 ...
 * Fetch random APODs
 */
export async function fetchRandomApods(count: number, thumbs: boolean = false): Promise<ApodEntry[]> {
 ...
 * Note: NASA API doesn't have built-in search, so we fetch and filter
 */
