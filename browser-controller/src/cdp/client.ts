...
export async function connect(options: ConnectOptions = {}): Promise<CDPClient> {
  ...
  const target = options.target || ((targets: CDP.Target[]) => {
...
export async function getBrowserVersion(host = 'localhost', port = 9222): Promise<{
  browser: string;
  version: string;
  userAgent: string;
}> {
...
export async function switchTab(targetId: string, host = 'localhost', port = 9222): Promise<void> {
  const targets = await listTargets(host, port);
...
export async function goBack(): Promise<void> {
  ...
  if (currentIndex > 0) {
    await currentClient.client.Page.navigateToHistoryEntry({ entryId: entries[currentIndex - 1].id });
    await currentClient.client.Page.loadEventFired();
  }
}
...
export async function scroll(direction: 'up' | 'down' | 'top' | 'bottom', amount = 500): Promise<void> {
  if (!currentClient) {
    throw new Error('Not connected to browser');
  }
